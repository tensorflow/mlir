...
public:
  ...
  void runOnFunction() override {
    ...
    ShapeInferenceInterface interface(&getContext());
    ...
    f.walk([&](mlir::Operation *op) {
      if (!interface.requiresShapeInference(op))
        return;
      if (interface.returnsGenericArray(op))
      {
    ...
    while (!opWorklist.empty()) {
      ...
      auto nextop = llvm::find_if(opWorklist, [&interface](Operation *op){return interface.returnsGenericArray(op);});
...
//===----------------------------------------------------------------------===//
// ShapeInferenceInterface
//===----------------------------------------------------------------------===//
namespace mlir {

ShapeInferenceInterface::~ShapeInferenceInterface() {}

bool ShapeInferenceInterface::returnsGenericArray(Operation *op) {
  auto *handler = getInterfaceFor(op);
  return handler ? handler->returnsGenericArray(op) : false;
}

bool ShapeInferenceInterface::requiresShapeInference(Operation *op) {
  auto *handler = getInterfaceFor(op);
  return handler ? handler->requiresShapeInference() : false;
}

void ShapeInferenceInterface::inferShape(Operation *op) {
  auto *handler = getInterfaceFor(op);
  assert(handler && "expected valid dialect handler");
  handler->inferShape(op);
}
